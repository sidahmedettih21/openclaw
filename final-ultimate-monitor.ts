// final-ultimate-monitor.ts — minimal, production-grade, no bloat
...
const logger = createLogger({
  ...
  transports: [new transports.Console({ format: format.printf(info => `[${info.timestamp}] ${info.level}: ${info.message}`) }), new transports.File({ filename: LOG, maxsize: 10_000_000, maxFiles: 2 })]
});

function csv(status: string, det="") { appendFileSync(CSV, `${new Date().toISOString()},${status},${det}\n`); }
...
const TARGET = "https://visas-pt.tlscontact.com/en-us/388184/workflow/appointment-booking?location=dzALG2pt";
const LOGIN = "https://visas-pt.tlscontact.com/en-us/login";
const APP_LIST = "https://visas-pt.tlscontact.com/en-us/application-list";
const CFG = {
...
const st = { ctx: null as BrowserContext|null, page: null as Page|null, running: false, lastHb: 0, lastSession: 0, errors: 0, circuitUntil: 0, found: false, total: 0 };
...
async function detect(p: Page): Promise<boolean> {
  ...
  if (!has) return false;
  // confirm slot is clickable
  return await p.evaluate(()=>!!document.querySelector('.slot-item:not(.unavailable), .available-slot, button[data-slot]'));
}

async function detectRetry(p: Page) {
  for (let i=0;i<2;i++) { // 2 attempts total
...
async function book(p: Page): Promise<boolean> {
  ...
  const clicked = await p.evaluate(()=>{
    for (const s of ['.slot-item:not(.unavailable)','.available-slot','button[data-slot]','td.available a']) {
...
async function cycle() {
  ...
  try {
    if (!await fetch("https://1.1.1.1", { method:"HEAD", signal:AbortSignal.timeout(3000) }).then(()=>true).catch(()=>false)) { csv("NO_INTERNET"); unlock(); return; }
    ...
    if (blocked) { trip(CFG.FATAL_BACKOFF); csv("BLOCKED"); unlock(); return; }
    if (!await detectRetry(page)) { csv("NO_SLOT"); st.errors=0; unlock(); return; }
    csv("SLOT_FOUND");
...
async function delay(min:number,max:number){ await new Promise(r=>setTimeout(r,Math.random()*(max-min)+min)); }

function schedule() {
  setTimeout(async ()=>{ await cycle(); schedule(); }, interval());
}
